...
在**InnoDB**中，创建一个新事务后，执行**第一个**select**语句**的时候，**InnoDB**会创建一个快照（**ReadView**），快照中会保存系统当前不应该被本事务看到的其他活跃事务id列表（即**m_ids**）。当用户在这个事务中要读取某个记录行的时候，**InnoDB**会将该记录行的**DB_TRX_ID**与该**ReadView**中的一些变量进行比较，判断是否满足可见性条件。

假设当前事务要读取某一个记录行，该记录行的**DB_TRX_ID**（即最新修改该行的事务ID）为**trx_id**，**ReadView**的活跃事务列表**m_ids**中最早的事务ID为**m_up_limit_id**，将在生成这个**Read Vew**时系统出现过的最大的事务ID+1记为**m_low_limit_id**（即还未分配的事务ID）。
